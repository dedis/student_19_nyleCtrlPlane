...
One of the principal reason to split the system based on locality is because of most regular transactions between people are locals, therefore one wants to ensure that in case of a global partition, the locals transactions can still be processed flawlessly. Following this hypothesis, one can try to use this structure to update the system. Based on that, there should be more interaction in local regions. Most interactions can be processed locally. But some need to be propagated up the hierarchy up to the global system. 

Different levels can have different time as well, as local regions should be capable of validating transaction fast, maybe its possible to pull the transaction to regional and global levels using more time. In that way, if a client want to buy a coffee from the local merchant, it could have the local validation in second, but if I wants to do international payment this could be slowed down as they are supposed to happened less often and require more verification.
...
Region Shrink         & Might exclude other nodes                                                              & Node leaving detected by one node of the region          & Round of communication between node in the region             \\ \hdashline[0.5pt/5pt]
Region Growth         & Might add other nodes                                                                  & Node entering detected by one node of the region         & Round of communication between node in the region             \\ \hdashline[0.5pt/5pt]
