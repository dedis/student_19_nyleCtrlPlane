...
\textit{open-membership} is that there should be a way to know the participants of the system if one wants to have a consensus.   A second challenge is that nodes can join, leaves or move in the system
at any moment. This might not be a problem for classic distributed ledger as
...
inverse situation as before, after a while some regions might not contains any node
that are maintaining the system. To avoid this situation, one might want to
...
if the region assignment is fixed but the nodes are moving, a node drift appart
...
the algorithm that allow to draw regions based on compact graph theory and which is described in
...
slightly orthogonal, as the Controle Plane is not directly linked to the replicated blockchain.
...
Monoxide \cite{Wang2019} and Stellar \cite{Lokhava2019}, but they don't use the concept of the
...
its implementation and performance analysis some drawbacks are put in light. Then a series of strawman
...
Some concepts are directly inspired from these works. The
Sybil-resistance scheme used in the registration system is directly inspired
from DFINITY \cite{Hanke2018}, using \textit{endorsement} in the general way, which can
be in practice replaced by any Sybil-resistance scheme like Proof-of-Work
\cite{Nakamoto2009}, Proof-of-Stake \cite{wood2014ethereum}, or even Proof-of-Personhood \cite{Borge2017}. In particular this work tries to solve some drawbacks of
...
resistant to WWIII scenarios. Byzcoin \cite{Kogias2016} and Omniledger
\cite{Kokoris-Kogias2017} gives another interesting solution to accelerate the validation, but their results are orthogonal to this research. Stellar's
...
are described in detail in \autoref{chap:Background}. There is a class of algorithm that
...
partition is maintained. To reduce this overhead, CRUX 
algorithm for regions creation \cite{Basescu2014} presented below ensure that the proper number of
...
\cite{Basescu2014} is used for a specific system, overhead can be even
more reduced:  as the systems are replicated in every region, most of the data
is replicated as well. So one might actually dig inside the
...
Nyle uses ARA as the representation of one region. In each of these regions there will be a copy of the same
...
distance between two nodes. This is the case for the Internet network \cite{Seibert2014}. On this result we build the
...
\subsection{Locality Component}
 ...
 regions. This part will be based on CRUX. At each epoch
 ...
 epoch. If nodes have crashed, they won't be able to join for the next epoch and will therefore leave the system.
...
system as they will know the participants, the pings distances between each one of them and their levels. Therefore these participants will be
...
plane, node insertion is possible at the beginning of every epoch \autoref{fig:insertion-comparision}.
...
Taking over the lottery process should not be possible by design.
Indeed, the lottery is based on a public source of randomness that renewed at
each epoch, and revealed after the registration of the levels. Nodes can know the level of other nodes, because they base the compute of their
...
idea of this Strawman model is to do the same by limiting modifications of regions from one epoch
...
the current epoch using the less redrawing as possible. Registration is still
...
nodes keep their levels from one epoch to the next.  However some small
...
appendix [\autoref{app:unbalanced-levels}]. The lottery process presented in \autoref{sec:common-tools}, is a bit changed in
...
of nodes that one should have at every level as it mentioned in \autoref{example-lottery}. 

Instead of drawing directly the levels from a randomness source, nodes will
draw a random number from this source between 0 and 1 [\autoref{fig:sketch-new-levels}].  All nodes can deduce
...
This section describes the reasons for keeping the levels. In order to understand what are the effects on the global system it is useful to look in detail at what are the consequences of the movement of a given node have on a given fixed node. Then some precise examples of evolution of the system are treated as well. 

\subsubsection{Effects for a fixed node} 
The point of view of one node that stays fixed in the system is taken, the goal is that node can keep most of its
region assignment.  Other nodes might
...
Assume that going from Epoch $i$ to $i+1$, one of a relatively high-level node as gone from one place to
...
One can think that the levels of joining node might have a big influence on the system, this part try to illustrate what might happen.
...
quantity increases in $O(N^2)$, where $N$ is the number of node. Consensus might become too costly for that reason.
...
\subsection{Protocol}
The protocol is mostly the same as the simple control plane protocol [\autoref{fig:registrationprotocol}]. The only difference is the consensus on the
...
this structure to update the system. As there should be more
interaction in local regions. Most interactions can be processed locally. 
...
system. What type of information can be inferred from this additional method ? First
...
\subsubsection{Orthogonal other example}
If one wants to leverage the locality of interactions to build regions it might be worth it to investigate the following case. 
Imagine that some nodes $A$ and $B$ are close locally and interacts often, but they are not part of the same small local region. 
By design there is a bigger region in which they interacts, and each of these interaction should pass by this bigger region. One property that might be useful is that these frequent interactions should have an impact on the system leading to the inclusion of one node in a "regional" region. If the metric that defines distance is changed from kilometers to "interactions". One should be able to redraw the whole system based on that, and to apply Crux to solves the system. Now how to actually define this metric ? Let's try with count of operation per unit of time.  Let's enumerate the properties of a metric space. 

\begin{itemize}
\item The distance from a point to itself is zero
\item The distance between two distinct points is positive
\item The distance from A to B is the same as the distance from B to A
\item The distance from A to B (directly) is less than or equal to the distance from A to B via any third point C.
\end{itemize}

\color{red} TODO cite metric space \color{black}

Interestingly the interaction metric follow this property. Indeed for the first, one can consider that one node always interacts with itself. Second the number of interaction per unit of time is positive. Third the interactions are counted as symmetric (if $A$ is sending a message to $B$ we count that as an interaction between $A$ and $B$). Fourth the number of interaction between two nodes is always less or equal that the number of interaction that transit to a third point. Now the problem with this metric is that it is actually not transitive. First one should notice that if $A$ is close to $B$ and $C$ it is possible that $B$ and $C$ never interacts therefore are "far from each other". 
...
This can be used to adapt the system.The rules of thumb is that node should only care of what is important for them.
...
% TODO Threat Analysis : This might be prone to attacks. Might not need this round of communication, base everything on the pings. 
...
\begin{sidewaystable}
\centering
\begin{tabular}{m{0.15\textwidth}m{0.3\textwidth}*{2}{>{\arraybackslash}m{0.2\textwidth}}}
\toprule
