...
func init() {
	onet.SimulationRegister(gpr.Name, NewSimulationProtocol)
}
...
func (s *SimulationProtocol) Run(config *onet.SimulationConfig) error {
	...
	for round := 0; round < s.Rounds; round++ {
		...
		p, err := config.Overlay.CreateProtocol(gpr.Name, config.Tree,
			onet.NilServiceID)
		if err != nil {
			return err
		}
		ann := gpr.Announce{
			Signer: config.Roster.List[0].ID,
			Proof:  &gpr.SignatureResponse{},
			Epoch:  2,
		}
		p.(*gpr.GossipRegistationProtocol).Msg = ann

		go p.Start()

		timeout := 10 * time.Second
		select {
		case sum := <-p.(*gpr.GossipRegistationProtocol).ConfirmationsChan:
			round.Record()
			if size != sum {
				return errors.New("The number of confirmations is not the number of nodes")
			}
		case <-time.After(timeout):
			return errors.New("Didn't finish in time")
		}
